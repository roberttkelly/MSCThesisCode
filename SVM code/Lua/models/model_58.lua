...
createModel = function(nGPU)
	  ...
	  lowerModel:add(nn.JitterLayer(sampleSize[2], sampleSize[3], jitterFunc))
	  lowerModel:add(nn.WarpAffine(sampleSize[2], sampleSize[3]))
	  lowerModel:add(nn.GCN())

	  for i = 1,4 do 
		    lowerModel:add(makeConvLayer(features[i], features[i+1], 3, 1, 1))
			lowerModel:add(nn.VeryLeakyReLU(0,0.1))
			lowerModel:add(makeConvLayer(features[i+1], features[i + 1], 1, 1, 0))
			lowerModel:add(cudnn.SpatialMaxPooling(3, 3, 2, 2))
			lowerModel:add(nn.VeryLeakyReLU(0,0.1))
	  end
	  copyModule(saved_m46, lowerModel)
	  local switchLayer = nn.SwitchWrapper(lowerModel, false)
	  model:add(switchLayer)
	  model:add(nn.Dropout(0.1))
	  for i = 5,#features-1 do 
